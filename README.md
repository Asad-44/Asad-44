<div align="center">

# Muhammad Asad Shafiq

**Cybersecurity / Network Security Engineer**

Software Engineering student building a strong foundation in network security through hands-on labs, secure application development, and continuous self-directed learning.

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=500&size=22&duration=4000&pause=1000&color=58A6FF&center=true&vCenter=true&width=600&lines=Cybersecurity+Engineer;Network+Security+%C2%B7+DFIR+%C2%B7+Cloud+Security;Python+%C2%B7+Node.js+%C2%B7+Linux)](https://git.io/typing-svg)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/muhammad-asad-shafiq)
[![Profile Views](https://komarev.com/ghpvc/?username=Asad-44&abbreviated=true&color=58A6FF)](https://github.com/Asad-44)
</div>
<div align="center">
📧 asad.shafiq411@gmail.com
</div>

---

## About

- **Current:** Software Engineering student at **COMSATS University Islamabad**, Lahore Campus
- **Interests:** Network Engineering, Network security, cloud security, DFIR, SOC, and secure application development
- **Building:** A home cybersecurity lab covering attack simulation, IDS, and log automation
- **Goal:** Long-term career defending networks and infrastructure

---

## Featured Projects

<table>
<tr>
<td width="50%" valign="top">

### [LockBox](https://lockbox-5it9.onrender.com)
Zero-knowledge E2EE file sharing platform with client-side AES-256-GCM encryption via the Web Crypto API, PBKDF2 key derivation (200,000 iterations), and burn-on-read sharing. Servers store only ciphertext.

![Node.js](https://img.shields.io/badge/Node.js-05122A?style=flat-square&logo=node.js&logoColor=339933)
![Redis](https://img.shields.io/badge/Redis-05122A?style=flat-square&logo=redis&logoColor=DC382D)

</td>
<td width="50%" valign="top">

### [RideRevenue Tracker](https://github.com/Asad-44/riderevenue-backend.git)
Full-stack fleet management system with JWT authentication, role-based access control, and a normalized MySQL schema. Hardened against SQL injection with bcrypt password hashing and strict input validation.

![Node.js](https://img.shields.io/badge/Node.js-05122A?style=flat-square&logo=node.js&logoColor=339933)
![MySQL](https://img.shields.io/badge/MySQL-05122A?style=flat-square&logo=mysql&logoColor=4479A1)

</td>
</tr>
</table>

---

## Technical Skills

### Security & Networking

![Wireshark](https://img.shields.io/badge/Wireshark-05122A?style=for-the-badge&logo=wireshark&logoColor=1679A7)
![Nmap](https://img.shields.io/badge/Nmap-05122A?style=for-the-badge&logo=nmap&logoColor=FF6600)
![Suricata](https://img.shields.io/badge/Suricata-05122A?style=for-the-badge&logo=suricata&logoColor=D7263D)
![Tcpdump](https://img.shields.io/badge/Tcpdump-05122A?style=for-the-badge&logo=linux&logoColor=white)
![Hydra](https://img.shields.io/badge/Hydra-05122A?style=for-the-badge&logo=hydra&logoColor=white)
![Cisco Packet Tracer](https://img.shields.io/badge/Cisco_Packet_Tracer-05122A?style=for-the-badge&logo=cisco&logoColor=1BA0D7)

`OSI Model` `TCP/IP` `Subnetting` `DNS` `HTTP/S` `Network Protocols`

### Programming

![Python](https://img.shields.io/badge/Python-05122A?style=for-the-badge&logo=python&logoColor=3776AB)
![C++](https://img.shields.io/badge/C%2B%2B-05122A?style=for-the-badge&logo=cplusplus&logoColor=00599C)
![Java](https://img.shields.io/badge/Java-05122A?style=for-the-badge&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-05122A?style=for-the-badge&logo=javascript&logoColor=F7DF1E)

### Core CS

`OOP` `Data Structures & Algorithms` `File Handling`

### Tools

![Git](https://img.shields.io/badge/Git-05122A?style=for-the-badge&logo=git&logoColor=F05032)
![GitHub](https://img.shields.io/badge/GitHub-05122A?style=for-the-badge&logo=github&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-05122A?style=for-the-badge&logo=linux&logoColor=FCC624)

---

## Certifications

<a href="https://www.credly.com/badges/0ec7030d-f982-4343-9b5f-371387a7cb19/public_url">
  <img src="https://images.credly.com/size/680x680/images/ef9f86ad-4e53-4dbc-8951-e4cab81ff09e/blob" alt="Google Cybersecurity Certificate" width="120"/>
</a>

---

## Education

**BS Software Engineering** — COMSATS University Islamabad, Lahore Campus *(2024 – 2028)*

---

## Currently Building

🔒 A home cybersecurity lab for active-defense practice — attack simulation, IDS deployment with Suricata, and Python-based threat log parsing. [View the repo →](https://github.com/Asad-44/home-lab-cyber.git)

---
